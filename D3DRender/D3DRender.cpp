...
int iWidth = 800;
int iHeight = 600;
D3DRender::D3DRender(HINSTANCE hInst, int iWidth, int iHeight)
{
	hInstance = hInst;
	iWidth = iWidth;
	iHeight = iHeight;
}
bool D3DRender::StartRenderer()
{
	HRESULT hr = ::CoInitialize(nullptr);  // for WIC & COM
	if (FAILED(hr))
	{
		// 处理初始化失败的情况
		MessageBox(nullptr, L"Failed to initialize COM library.", L"Error", MB_OK);
		return false;
	}

	std::thread updateThread(&D3DRender::Update, this);
	updateThread.detach();
	return true;
}
void D3DRender::Update()
{
	...
	try
	{
		...
		UINT compileFlags = 0;
#endif
		TCHAR pszShaderFileName[] = _T("D:\\Users\\2333\\source\\repos\\PlayWindow\\D3DRender\\Texture.hlsl");
		...
		CD3DX12_HEAP_PROPERTIES stVertexBufferHeapProperties(D3D12_HEAP_TYPE_UPLOAD);;
		CD3DX12_RESOURCE_DESC stVertexBufferDesc = CD3DX12_RESOURCE_DESC::Buffer(nVertexBufferSize);
		GRS_THROW_IF_FAILED(pID3DDevice->CreateCommittedResource(
			&stVertexBufferHeapProperties,
		...
		const WCHAR* pszTexcuteFileName = _T("D:\\Users\\2333\\Desktop\\ac-congrats.bmp");
		...
		CD3DX12_HEAP_PROPERTIES stTextureHeapProperties(D3D12_HEAP_TYPE_DEFAULT);
		...
		CD3DX12_HEAP_PROPERTIES stUploadHeapProperties(D3D12_HEAP_TYPE_UPLOAD);
		CD3DX12_RESOURCE_DESC stUploadDesc = CD3DX12_RESOURCE_DESC::Buffer(n64UploadBufferSize);
		GRS_THROW_IF_FAILED(pID3DDevice->CreateCommittedResource(
			&stUploadHeapProperties,
			D3D12_HEAP_FLAG_NONE,
			&stUploadDesc,
			D3D12_RESOURCE_STATE_GENERIC_READ,
			nullptr,
		...
		while (!bExit)
		{
			...
			{//计时器时间到
				...
				CD3DX12_RESOURCE_BARRIER stResBar = CD3DX12_RESOURCE_BARRIER::Transition(pIARenderTargets[nFrameIndex].Get(), D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET);
				pICommandList->ResourceBarrier(1, &stResBar);
				...
				CD3DX12_RESOURCE_BARRIER stResBar2 = CD3DX12_RESOURCE_BARRIER::Transition(pIARenderTargets[nFrameIndex].Get(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT);
				...
				SetWindowText(hWnd, std::to_wstring(nFrame++).c_str());
			}
			break;
			case 1:
			{//处理消息
